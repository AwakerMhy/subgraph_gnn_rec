"""src/dataset/real/sx_mathoverflow.py — StackExchange MathOverflow 数据集预处理

数据来源：SNAP https://snap.stanford.edu/data/sx-mathoverflow.html
格式：每行 "src dst timestamp"（空格分隔，Unix 时间戳）
包含：回答/评论/提问三种交互类型，已合并为单一有向时序图
"""
...
class SXMathOverflowDataset(TemporalDataset):

    @property
    def name(self) -> str:
        return "sx_mathoverflow"

    def process(self) -> None:
        raw_path = self.raw_dir / "sx-mathoverflow" / "sx-mathoverflow.txt"
        ...
        print(f"[SX-MathOverflow] 预处理完成：{n_nodes} 节点，{len(edges)} 边")
