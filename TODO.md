...
- [ ] **子图结构消融：(u, v) 局部子图中 ego graph 非 v 邻居部分的贡献**
  - **问题**：在以 u 为中心的 ego graph 中，除目标节点 v 的直接连接外，u 的其他邻居（non-v neighbors）所构成的图结构对链接预测的贡献是什么？去掉这部分结构后 MRR 变化多少？
  - **实验设计**：构建两种子图变体：(1) 完整二度子图（当前默认）；(2) 掩盖 u 的非 v 邻居之间的边（仅保留 u→nbr 和 nbr→v 路径，去除 nbr 之间横向边）；对比两者在多数据集上的 MRR@1/5
  - **动机**：帮助理解 GNN 在子图内捕获的是"局部拓扑结构（三角形/共同邻居密度）"还是"路径特征"
