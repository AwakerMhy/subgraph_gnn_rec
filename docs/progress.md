...
[2026-05-03 12:30] [v2 全数据集 algo_sweep（9数据集×11算法，30轮，新配置）] [configs/online/algo_sweep_*/*, scripts/run_sweep_from_configs.py] [完成7/9，sx_askubuntu/sx_superuser进行中]

## v2 实验配置变更（2026-05-03）

相比旧版本的关键变化：init_strategy: stratified→random，user_selector.strategy: composite→uniform，total_rounds: 100→30，hidden_dim(gnn_sum/concat): 8→32，log_every: 50→1。结果目录加 _v2 后缀。

## v2 实验结论：GNN 各变体 vs 其他方法（30轮均值 MRR@1，7数据集）

### MRR@1 对比（30轮均值）

| 数据集 | gnn_sum | gnn_concat | gnn(last) | best启发式 | random |
|--------|---------|------------|-----------|-----------|--------|
| wiki_vote | 0.204 | 0.244 | 0.254 | aa=0.262 | 0.161 |
| sx_mathoverflow | 0.517 | 0.424 | 0.291 | aa=0.381 | 0.175 |
| slashdot | 0.434 | 0.479 | 0.331 | cn=0.379 | 0.231 |
| college_msg | 0.541 | 0.522 | 0.159 | pa=0.206 | 0.085 |
| bitcoin_alpha | 0.578 | 0.623 | 0.277 | aa=0.311 | 0.148 |
| dnc_email | 0.401 | 0.476 | 0.379 | aa=0.386 | 0.188 |
| email_eu | 0.574 | 0.541 | 0.347 | aa=0.454 | 0.202 |

### 核心结论

1. gnn_sum/gnn_concat 全面优于 random（MRR@1 提升 1.3–6倍）
2. GNN 明显赢启发式（差距>0.1）：sx_mathoverflow、college_msg、bitcoin_alpha、email_eu
3. GNN 小幅赢启发式（差距0.05–0.1）：slashdot、dnc_email
4. GNN 持平或略输：仅 wiki_vote（aa/cn 高 0.01–0.06）
5. gnn(last) 是最弱变体，无一数据集超过 gnn_sum/concat，不推荐
6. Coverage：GNN 普遍低于 CN/AA（GNN 精确但保守，CN/AA 覆盖广）
7. gnn_concat vs gnn_sum 无统一优劣，建议两者都报告
8. 旧结论推翻：bitcoin_alpha 和 email_eu 在新配置下 GNN 不再失败（composite→uniform 是关键）

[2026-05-06 16:00] [constant lr=1e-4 消融实验（8数据集×4GNN变体，seed=0）] [scripts/run_constant_lr_sweep.py(新建), scripts/compare_constant_lr.py(新建), configs/online/algo_sweep_*_v2_constlr_s0/] [进行中，sx_superuser/advogato待完成]
- constant lr=1e-4 对比 cosine_warmup lr=0.001
- gnn/gnn_sum/gnn_concat：const全面胜出（8/8、7/7、6/7），平均提升 +0.05~+0.11 MRR@1
- gnn_h32：混合结果（5胜3负），cosine在email_eu/sx_mathoverflow/college_msg更优
- wiki_vote首次出现正向改善（+0.04~+0.05），email_eu gnn提升最大（+0.26）
- 建议：gnn/gnn_sum/gnn_concat切换constant lr=1e-4为新默认；gnn_h32待进一步消融

[2026-05-06 18:30] [lr 三路消融完结（cosine/1e-4/5e-5 × 3变体 × 10数据集）] [scripts/run_constlr5e5_concat_sweep.py(新建), scripts/run_constlr5e5_gnn_sum_sweep.py(新建)] [完成]
- gnn(h8): 1e-4 constant（5e-5 vs 1e-4 各胜5个，无差异）
- gnn_h32: 5e-5 constant（1e-4偏大，4数据集cosine更优）
- gnn_concat: 5e-5 constant（7/10胜，sx_mathoverflow/sx_askubuntu差距仅0.009~0.014）
- gnn_sum: 1e-4 constant（9/10胜，5e-5在sx_askubuntu/-0.065严重退步）
- 规律：layer_sum梯度强耐高lr；layer_concat/h32+last需保守lr；h8不敏感
