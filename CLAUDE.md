...
**核心问题**：研究推荐系统与社交网络之间的反馈闭环——当推荐系统持续向用户推送候选连接、用户以随机概率接受或拒绝、接受的边写入网络并更新模型时，这一在线学习过程如何驱动网络结构演化，以及演化结果是否趋近真实熟人网络的统计特性。

**技术手段**：以基于二度邻居子图的 GNN（GIN + DRNL 标记）作为推荐模型的精排层，配合多路召回（AA/PPR/社区随机混合）、概率化用户反馈（$p_{\text{pos}}/p_{\text{neg}}$ 伯努利模型）和在线微调（BCE 增量更新），构成完整的在线仿真环路。边时间戳仅用于初始图的构建与切分，不作为模型输入特征。
