# Social Network Online Learning Simulation

> 最后更新：2026-04-25
>
> **项目目标**：研究推荐系统驱动的社交网络在线演化过程。核心问题：当推荐模型持续推送候选连接、用户以概率性反馈接受或拒绝、接受边实时写入网络并触发模型在线微调时，这一反馈闭环能否推动网络结构收敛至真实熟人网络的统计特性？GNN 链接预测（基于二度邻居子图的 GIN + DRNL）是推荐系统的精排组件，服务于在线仿真这一核心问题。边时间戳仅用于初始截断图构建，不作为模型输入特征。
...
1. **用户采样**：从当前图 $\mathcal{G}_r$ 中按 Composite 策略（幂律活跃度 × 度数 × 时间衰减）选取活跃用户 $u$
2. **候选召回**：多路召回器（AA + PPR + 社区随机，配额 30:10:10）生成候选集 $\mathcal{C}_u$
3. **精排推荐**：GNN 评分模型对 $\mathcal{C}_u$ 排序，取 Top-$K$ 推送给 $u$
4. **用户反馈**：对 $(u,v) \in \text{Top-}K$，若 $(u,v) \in G^*$ 则以 $p_{\text{pos}}$ 接受，否则以 $p_{\text{neg}}$ 接受（伯努利噪声模拟真实不确定性）
5. **图更新**：接受的边写入 $\mathcal{G}_r$，Cooldown 压制短期重复推荐
6. **模型微调**：以接受/拒绝对构造 BCE 损失，增量更新模型参数

**研究问题**：随轮次推进，MRR@K、候选池 Coverage、G\* 命中率等指标如何演化？网络结构是否收敛？
...
- **子图**：提取 $\{u\} \cup N(u) \cup \text{CN}(u,v) \cup \{v\}$ 所诱导的有向子图 $\mathcal{S}_{uv}$（ego-graph + 公共邻居设计）
- **输出**：标量评分 $s_{uv} \in (0,1)$，越高表示 $u$ 在下一轮向 $v$ 建边的概率越大
