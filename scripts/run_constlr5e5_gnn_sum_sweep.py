"""gnn 和 gnn_sum constant lr=5e-5 消融：验证是否优于 1e-4。
...
if __name__ == "__main__":
    ...
    tasks = [(ds, al, sd) for sd in seeds for ds in DATASETS for al in ALGOS]
    pending = [t for t in tasks
               if not (Path(f"results/online/{sweep_name(t[0], t[2])}/{t[0]}_{t[1]}/rounds.csv")).exists()]

    print(f"lr={LR}  algos={ALGOS}  共 {len(tasks)} 个，跳过 {len(tasks)-len(pending)} 个，待跑 {len(pending)} 个，workers={args.workers}", flush=True)
